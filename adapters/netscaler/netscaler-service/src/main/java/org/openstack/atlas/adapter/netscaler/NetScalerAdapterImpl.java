...
public class NetScalerAdapterImpl implements ReverseProxyLoadBalancerAdapter {
            ...
            throws RemoteException, InsufficientRequestException, AdapterRollBackException 
    {
    	...
    	int port = lb.getPort().intValue();
    	
    	String vipType = null;
    	
    	for (LoadBalancerJoinVip lbvip : lb.getLoadBalancerJoinVipSet())
    	{
    		vipType = lbvip.getVirtualIp().getVipType().toString();
    		break;
    	}
    	...
    	String persistenceType = lb.getSessionPersistence().toString();

    	
    	String request = "<loadBalancer ";
    	request += "name=\"" + name + "\"" + " ";
    	request += "port=\"" + port + "\"" + " ";
    	...
    	if (vipType != null) 
    	{
    	    request += "<virtualIps><virtualIp type=\"" + vipType + "\" /></virtualIps>";
    	}
    	...
    	if (nodes != null)
    	{
    		...
    		{
    			request += "<node ";
    			request += "address=\"" + node.getIpAddress()  + "\"" + " ";
    			request += "port=\"" + node.getPort() + "\"" + " ";
    			request += "condition=\"" + node.getCondition().toString() + "\"" + " ";
    			request += "/>";
    		}
    	...
    	if (monitor != null) 
    	{
    	    ...
    	    request += "type=\"" + monitor.getType() + "\"" + " ";
    	    request += "delay=\"" + monitor.getDelay() + "\"" + " ";
    	    request += "timeout=\"" + monitor.getTimeout() + "\"" + " ";
    	    request += "attemptsBeforeDeactivation=\"" +  monitor.getAttemptsBeforeDeactivation() + "\"" + " ";
    	    
    	    if ((monitor.getType() == HealthMonitorType.HTTP) || (monitor.getType() == HealthMonitorType.HTTPS))
    	    {
    	    	request += "path=\"" + monitor.getPath() + "\"" + " ";
    	    	request += "statusRegex=\"" + monitor.getStatusRegex() + "\"" + " ";
    	    	request += "bodyRegex=\"" + monitor.getBodyRegex() + "\"" + " ";
    	    }
    	...
    	/*
    	request += "<sessionPersistence persistenceType=\"" + persistenceType + "\"" + " />";
    	*/
    	...
    	String host = lb.getHost().getEndpoint();
    	String urlStr = host + "/" + lb.getAccountId() + "/loadbalancers";
    	
    	LOG.debug(String.format("service URL string: '%s'...", urlStr));

    	LOG.debug(String.format("Creating load balancer request: '%s'...", request));
    	...
    	try {
    	    NSRequest.perform_request("POST", urlStr, headers, request);
    	} catch (Exception e)
    	{
    		LOG.debug(e.getMessage());
    		throw new RemoteException("Exception occurred: " + e.getMessage());    		
    	}
    }
